...
abstract class RetryMarkerMixin {
    ...
    private static final int MIN_DELAY = 20 * 60; // Mojang default 40.
    private static final int ATTEMPT_DURATION = 20 * 60 * 5; // Mojang default 400.
    ...
    @Shadow
    public abstract void setAttemptTime(long time);

    @Shadow
    public abstract boolean isAttempting(long time);

    @Shadow
    public abstract boolean shouldRetry(long time);

    @ModifyConstant(method = "setAttemptTime(J)V", constant = @Constant(intValue = 40))
    private int replaceMinDelayInSetAttemptTime(int value) {
        return MIN_DELAY;
    }

    @ModifyConstant(method = "setAttemptTime(J)V", constant = @Constant(intValue = 400))
    private int replaceAttemptDurationInSetAttemptTime(int value) {
        return ATTEMPT_DURATION;
    }

    @ModifyConstant(method = "isAttempting(J)Z", constant = @Constant(intValue = 400))
    private int replaceAttemptDurationInIsAttempting(int value) {
        return ATTEMPT_DURATION;
    }
}