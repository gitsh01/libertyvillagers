...
public abstract class VillagerBreedTaskMixin extends Task<VillagerEntity> {
    ...
    @Shadow
    abstract boolean canReachHome(VillagerEntity villager, BlockPos pos);
    ...
    private void goHome(ServerWorld world, VillagerEntity first, VillagerEntity second, CallbackInfo ci) {
        if (CONFIG.villagersGeneralConfig.villagerBabiesRequireWorkstationAndBed) {
            Optional<BlockPos> optionalWorkstation = world.getPointOfInterestStorage()
                    .getPosition(VillagerProfession.NONE.getWorkStation().getCompletionCondition(),
                            (pos) -> this.canReachHome(first, pos), first.getBlockPos(),
